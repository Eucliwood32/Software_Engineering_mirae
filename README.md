...
---
# QCE — 부탁해 꼬마선장 테스트 케이스(test-cases.md) 요약
...
| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| FR-1.1 | `.pptx` / `.docx` / `.hwpx` 파일에서 공백·개행 제외 유효 글자 수와 도형 개수 추출. 손상 파일은 skip 후 경고 표시 | **[TC-FR-1.1-01]** docx 파일 유효 글자 수 정확성 검증<br>**[TC-FR-1.1-06]** 손상 파일 발견 시 예외 없이 skip 및 경고창에 파일명 포함 검증 | 🟢 |
| FR-1.2 | 파일 메타데이터(core_properties) 기반 작성자별 글자 수 집계. 메타 없으면 `"Unknown"` 분류 | **[TC-FR-1.2-01]** core_properties 기반 다중 파일 작성자별 분리 집계 검증<br>**[TC-FR-1.2-02]** 작성자 메타 부재 시 "Unknown" 분류 검증 | 🟢 |
| FR-1.3 | 분석 완료 후 결과 화면에서 조장이 여러 식별자를 1인으로 수동 병합(N:1). 자동 병합 금지. 시스템은 병합 후보를 제안할 수 있으나 확정은 조장만 | **[TC-FR-1.3-01]** Git/카톡 등 이기종 식별자의 N:1 수동 합산 검증<br>**[TC-FR-1.3-07]** AliasExtractor를 활용한 결정론적 군집 추천 제안 검증 | 🟢 |

### FR-2. Git 분석

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| FR-2.1 | 로컬 Git 저장소에서 작성자별 커밋 수·추가 라인·삭제 라인 수집. 잘못된 경로 → 빈 결과 반환, 예외 전파 없음. 50,000커밋 기준 30초 이내 완료 | **[TC-FR-2.1-01]** 작성자 이메일별 커밋/추가/삭제 라인의 정확한 매치 검증<br>**[TC-FR-2.1-02]** 잘못된 경로 입력 시 crash 없이 빈 결과(`{}`) 반환 검증 | 🟢 |
| FR-2.2 | 앱 기동 시 Git 설치 여부 점검. 미설치 시 메인 윈도우 표시 전 모달 안내 팝업 표시 후 Git 기능만 비활성화, 앱은 계속 실행 | **[TC-FR-2.2-01]** Git 명령어 미부재 환경 감지 로직 검증<br>**[TC-FR-2.2-05]** 미설치 상태 기동 시 메인 윈도우 노출 전 모달 팝업 안내창 출현 검증 | 🟢 |

### FR-3. 메신저 분석

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| FR-3.1 | 카카오톡 `.txt` 내보내기 파일을 발화자·시각·메시지 구조로 파싱. UTF-8 / CP949 인코딩 모두 지원 | **[TC-FR-3.1-01]** 정상 대화 줄의 발화자/메시지 정규화 분리 검증<br>**[TC-FR-3.1-06]** 문서나 Git 없이 카카오톡 단독 데이터 입력 시에도 파이프라인 정상 구동 검증 | 🟢 |
| FR-3.2 | 파싱 실패 줄은 카운트하며 skip. 전체 오염 시에도 예외 없이 `{records: [], skipped: N}` 반환 | **[TC-FR-3.2-01]** 정상 및 오염 줄 혼재 시 정확한 skip 카운트 작동 검증<br>**[TC-FR-3.2-02]** 100% 가비지 데이터 입력 시 crash 없는 방어적 처리 검증 | 🟢 |
| FR-3.3 | 단순 리액션(`ㅇㅇ`, `ㅋㅋ` 등)·미디어 태그(`(이모티콘)`, `(사진)` 등)·1글자 응답(`네`, `응` 등) 자동 불용어 처리. 사용자 편집 불가 | **[TC-FR-3.3-01]** 단순 초성 리액션 및 미디어 태그의 순수 소통량 제외 처리 검증<br>**[TC-FR-3.3-06]** StopwordFilter 내 외부 편집 API 원천 배제 검증 | 🟢 |

### FR-4. 정규화 및 가중치

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| FR-4.1 | Max 정규화: `값 / 최댓값` (최댓값 0이면 전원 0.0). 결과 소수점 4자리 반올림 | **[TC-FR-4.1-01]** 최댓값 대비 비율 계산 알고리즘 검증<br>**[TC-FR-4.1-02]** 최댓값이 0일 때 전원 0.0 처리 및 ZeroDivision 방지 검증 | 🟢 |
| FR-4.2 | 단일 커밋 추가 라인 50,000줄 초과 시 50,000으로 Capping. 합산 후 로그 스케일링→정규화. Capping 발생 커밋은 조장에게 목록 표시 | **[TC-FR-4.2-01]** 대형 복붙(5만 줄 초과) 커밋의 상한선 제한 처리 검증<br>**[TC-FR-4.2-06]** 합산 후 log1p 기반 로그 스케일링 반영 검증 | 🟢 |
| FR-4.2b | 특정 작성자의 단기 커밋 빈도가 평소 일평균 대비 3배 초과 시 이상 신호(작성자·기간·커밋 수·평소 평균) 표시 | **[TC-FR-4.2b-01]** 마감일 몰아치기(Burst Commit) 구간의 이상 신호 및 세부 항목 검출 검증 | 🟢 |
| FR-4.2c | 표시된 이상 신호를 조장이 개별적으로 "정상으로 표시" 가능. 점수에는 영향 없음(표시 전용). 이력은 세션 내에서만 유지 | **[TC-FR-4.2c-01]** 신호 예외 처리(Dismiss) 상태 관리 로직 검증<br>**[TC-FR-4.2c-03]** 세션 내 해제 시 신호 카드 UI 동기화 및 원본 점수 불변성 검증 | 🟢 |
| FR-4.2d | 정규화 지표 중 Z-Score -1.5 이하 항목이 2개 이상인 팀원을 하위 이상치 신호로 표시 | **[TC-FR-4.2d-01]** 복수 지표 저조 팀원의 하위 이상치 자동 탐색 검증<br>**[TC-FR-4.2d-03]** 산점도 차트 상에 경고(⚠) 오버레이 강조 연동 검증 | 🟢 |
| FR-4.3 | 임의 데이터 소스 부재 시 해당 가중치를 0으로 만들고 나머지를 상대 비율로 재조정. 재조정 후 합계 1.0(±0.0001) 보장. 3개 소스 모두 부재 시 분석 차단 | **[TC-FR-4.3-01]** 특정 플랫폼 데이터 결측 시 남은 소스의 상대 비율 재조정 검증<br>**[TC-FR-4.3-07]** 전량 결측 시 분석 차단 및 안내 배너 출력 검증 | 🟢 |
| FR-4.4 | 프리셋 3종(개발 중심 0.60/0.25/0.15 · 기획 중심 0.20/0.60/0.20 · 균형 설정 0.40/0.40/0.20) 또는 슬라이더(0%~100%, 5% step)로 가중치 조정. 합계 ≠ 100%이면 [분석 시작] 비활성화 | **[TC-FR-4.4-06]** UI 프리셋 버튼 클릭 시 슬라이더 값 동기 반영 검증<br>**[TC-FR-4.4-15]** 단일 슬라이더 조작 시 타 슬라이더의 실시간 자동 비례 연동 검증 | 🟢 |

### FR-5. 시각화 및 리포트

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| FR-5.1 | 분석 완료 시 막대 차트·레이더 차트·산점도 3종 동시 갱신. 각각 독립 위젯 클래스로 구현, 차트 간 직접 참조 금지 | **[TC-FR-5.1-01]** 데이터 미적재 상태 시 "분석할 데이터가 없습니다." 안내 문구 검증<br>**[TC-FR-5.1-03]** 분석 완료 트리거 시 3종 차트 컴포넌트의 동시 동기화 갱신 검증 | 🟢 |
| FR-5.1a | **막대 차트**: 팀원별 종합 기여 지표 표시. hover 툴팁(6항목)·팀 평균선·진입 애니메이션(20프레임, 30ms) 포함 | **[TC-FR-5.1a-03]** hover 시 정규화 점수 및 원시 정량 지표(라인/글자/발화 수) 포함 6개 항목 툴팁 검증<br>**[test_bar_average_line]** 수평 평균선 수치 일치 검증 | 🟢 |
| FR-5.1b | **레이더 차트**: Git·문서·메신저 3축(또는 소스별 세부 최대 9축). 범례 토글·팀 평균 폴리곤·결측 축 점선 처리 포함 | **[TC-FR-5.1b-05]** 범례 클릭 시 해당 팀원 폴리곤 visibility 토글 검증<br>**[TC-FR-5.1b-08]** 제외된 데이터 소스 축의 점선 가시화 및 레이블 검증 | 🟢 |
| FR-5.1c | **산점도**: 입력 소스 수에 따라 동적 구성. FR-4.2d 이상치 팀원 시각 강조. 점 클릭 시 레이더 하이라이트 연동 | **[TC-FR-5.1c-08]** 특정 데이터 점 클릭 시 레이더 차트 굵기 강화 Signal 연동 검증<br>**[TC-FR-5.1c-09]** Z-Score 하위 팀원 점에 대한 경고 오버레이 강조 검증 | 🟢 |
| FR-5.1d | 차트Acceptance: 막대·레이더·산점도 위젯의 데이터 계약 및 내부 수치 검증용 자동화 테스트 | **[test_bar_animation_final_height]** 애니메이션 종료 후 최종 타겟 지표 도달 확인 등 **12개 핵심 UI Acceptance Test 전량 통과** | 🟢 |
| FR-5.2 | `.md` / `.csv` 리포트 저장. CSV는 UTF-8 BOM으로 Excel 한글 호환. 저장 성공 시 상태바 3초 알림 | **[TC-FR-5.2-02]** Excel 호환성 확보를 위한 CSV 바이트 서두 BOM(`\xef\xbb\xbf`) 검증<br>**[TC-FR-5.2-04]** 산출 지표 명칭에 주관적 판정 뉘앙스 배제 검증 | 🟢 |
| FR-5.3 | 데이터 소스 결측 시 노란 배너 경고 + `.md` 블록쿼트 + `.csv` WARNING 행으로 동일 문구 출력 | **[TC-FR-5.3-01]** UI 상단 노란 경고 배너 출현 검증<br>**[TC-FR-5.3-02, 03]** 리포트 형식별 지정된 표준 결측 문구(Blockquote, WARNING 행) 출력 검증 | 🟢 |
| FR-5.4 | 단일 윈도우 내 **제출 → 로딩 → 결과** 3-스크린 전환. [새 분석] 시 기존 입력 데이터 전체 초기화 | **[TC-FR-5.4-01]** QStackedWidget 기반 스크린 내비게이션 검증<br>**[TC-FR-5.4-07]** [새 분석] 버튼 요청 시 이전 경로 및 적재 피드백 완전 초기화 검증 | 🟢 |
| FR-5.5 | 제출 화면: 로고·설명·멀티포맷 드래그앤드롭 존(`.pptx/.docx/.hwpx/.txt`)·적재 목록(개별 삭제 가능)·Git 저장소 선택·가중치 설정 | **[TC-FR-5.5-02]** 확장자 분류 처리를 통한 드래그 앤 드롭 파일 적재 및 파일명 표시 검증<br>**[TC-FR-5.5-09]** 미적재 상태 시 데이터 소스별 "없음" 라벨 노출 검증 | 🟢 |
| FR-5.6 | 로딩 화면: 분석 시작 1초 이내 진행률 표시. 완료 시 결과 화면, 오류 시 제출 화면으로 전환 | **[TC-FR-5.6-01]** 비동기 분석 진입 직후 프로그레스바 시각화 및 초기화 검증 | 🟢 |
| FR-5.7 | 결과 화면에서 여러 계정을 1인으로 병합(원시 지표 재집계→재정규화→차트 전체 갱신). 미선택 시 확인 버튼 비활성화. 취소 시 기존 화면 유지 | **[TC-FR-5.7-03]** 사후 병합 시 원시 데이터 재합산 및 전체 재정규화 알고리즘 연동 검증<br>**[TC-FR-5.7-07]** 미선택(0건 선택) 시 매핑 완료 OK 버튼 비활성화 가드 검증 | 🟢 |
| FR-5.8 | 다크 모드 지원: 시스템 자동 감지 + 수동 오버라이드(설정 다이얼로그). 차트 테마도 동기 갱신 | **[TC-FR-5.8]** 윈도우 다크/라이트 테마 변경 감지 및 Matplotlib 차트 스타일 동기화 갱신 검증 | 🟢 |
...
| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| NFR-1.1 | 분석 연산은 Worker Thread에서 실행. 5,000줄 분석 중 메인 윈도우 드래그 가능. 진행률 1초 이내 출현 | **[TC-NFR-1.1-01]** 대용량(5,000줄) 분석 중 메인 윈도우의 이벤트 처리가 멈추지 않는지(프리징 방지) 검증 | 🟢 |
| NFR-1.2 | 분석 중 중복 실행 차단. 시작 0.5초 이내 버튼 비활성화. 비정상 종료 후에도 버튼 재활성화 보장 | **[TC-NFR-1.2-02]** 분석 버튼 5회 연속 호출 시 Worker 스레드가 단 1개만 실행되어 중복 실행을 완벽히 차단하는지 검증 | 🟢 |
| NFR-1.3 | 동일 입력·가중치에 대해 결과가 항상 동일(결정론). 불용어 분류 결과도 동일 | **[TC-NFR-1.3-01]** 동일한 입력과 가중치로 2회 실행 시 결과값이 오차 없이 완전히 일치하는지 검증 | 🟢 |

### NFR-2. 보안 및 프라이버시

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| NFR-2.1 | 분석 대상 파일은 읽기 전용(`r`/`rb`)으로만 접근. 원본 수정 타임스탬프 분석 전후 동일 | **[TC-NFR-2.1-01]** 분석 전과 후, 원본 파일의 수정 타임스탬프(`getmtime`)가 완벽히 동일한지 확인하여 읽기 전용 제약 검증 | 🟢 |
| NFR-2.2 | 네트워크 송신 0건. `requests`·`urllib`·`httpx`·`socket`·`http.client` import 금지. 외부 링크는 `webbrowser.open()` (OS 위임)만 허용 | **[TC-NFR-2.2-02]** 네트워크 통신이 강제 차단된(`_no_network`) 격리 환경 하에서도 분석 파이프라인이 정상 작동하는지 검증 | 🟢 |
| NFR-2.3 | 캐시(`.qce_cache`)는 JSON만 사용(`pickle` 전면 금지). 저장 허용: 팀원 식별자·정규화 점수·타임스탬프·가중치. 저장 금지: 메시지 본문·소스코드 내용. 원자적 쓰기(tmp → fsync → replace) | **[TC-NFR-2.3-05]** 캐시 저장 직후 임시 파일(`.tmp`)이 디스크에 전혀 잔존하지 않음을 확인하여 원자적 쓰기(Atomic Write) 안전성 검증 | 🟢 |
| NFR-2.4 | 원본 파일 없이 캐시만으로 이전 결과 복원 가능. 상태바에 캐시 로드 안내 및 분석 일시 표시 | **[TC-NFR-2.4-01]** 원본 데이터 전체 삭제 후 캐시만 단독 로드하여도 차트와 점수가 정상 복원되는지 검증 | 🟢 |

### NFR-3. 안정성

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| NFR-3.1 | 텍스트 파일 인코딩 자동 감지: UTF-8 → CP949 순 시도. 둘 다 실패 시 해당 파일 skip + error 키 반환, 앱 계속 실행 | **[TC-NFR-3.1-01, 02]** UTF-8 및 CP949 인코딩으로 작성된 한글 파일의 글자를 깨짐 없이 자동 감지하여 추출해 내는지 검증 | 🟢 |
| NFR-3.2 | Git·OOXML·메신저 분석 모듈 상호 격리. 임의 1개 모듈 삭제·실패 시 나머지 2개 모듈 단위 테스트 정상 통과. 모듈 간 직접 import 금지, 상위 오케스트레이터(`analyzer.py`) 경유 | **[TC-NFR-3.2-03]** 1개의 데이터 모듈에서 강제 에러(Mock) 발생 시, 프로세스가 죽지 않고 나머지 2개 모듈의 분석 결과가 최종 결과 화면에 정상 반영되는지 검증 | 🟢 |

### NFR-4. 배포

| ID | 요약 | 주요 검증 (Test Case) | 통과 여부 |
|---|---|---|:---:|
| NFR-4.1 | PyInstaller `--onefile --windowed`로 단일 실행 파일(`QCE.exe`) 빌드. QCE 전용 아이콘(`.ico`) 적용 | **[D-01]** 빌드된 단일 `QCE.exe` 실행 파일에 꼬마선장 전용 아이콘(`.ico`)이 윈도우 OS상에서 올바르게 노출되는지 수동 시스템 검증 | 🟢 |
