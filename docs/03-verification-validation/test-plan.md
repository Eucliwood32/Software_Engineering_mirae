...
- **차트 렌더 검증 한계:** matplotlib 픽셀 단위 비교는 불안정하므로, **데이터 계약(축 레이블·평균선 Y값·점 색상 매핑·Signal 발행·툴팁 동적 항목)** 을 검증하고 시각적 완성도는 L4 수동으로 보완한다.
...
| **v1.4** | **2026-06-02** | **Capping 한도 상향 및 2차 UI/UX 개선 반영 사항 동기화 (전역 문서 업데이트의 일환).** | 이대한, 김휘중 공동 작업 |