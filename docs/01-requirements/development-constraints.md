...
## 1. 문서 개요
본 문서는 QCE 시스템의 설계, 구현, 배포 전반에 걸쳐 절대적으로 준수해야 하는 **기술적·환경적 제약사항(Constraints)**을 정의합니다. 본 문서에 명시된 제약은 기능적/비기능적 요구사항을 구현할 때 타협할 수 없는 물리적, 구조적 한계를 의미합니다. (※ 비기능적 요구사항은 `requirements-record.md`를 참조하십시오.)
...
| **C-9** | **Git CLI 의존성 대응**<br>(Git CLI Prerequisite) | 로컬 저장소 분석을 위해 사용자 OS에 `Git 2.x` CLI가 설치되고 PATH에 등록되어 있어야 합니다. 단, 미설치 환경에서도 앱이 비정상 종료(Crash)되지 않고 기능을 우회할 수 있도록 에러 핸들링이 강제됩니다. |