...
| 디자인 가이드 | qce-design-guide.md v1.0 (Product-First Minimalism — 색·타이포·간격·반경·그림자 토큰의 단일 출처) |
...
└── view/
    ...
    ├── style/
    │   ├── tokens.py               # 색상·폰트·간격·반경·그림자 토큰 (라이트/다크 팔레트, v3.0 design-guide)
    │   ├── theme.py                # ThemeManager (시스템 연동·라이트/다크 전환, v2.0)
    │   ├── effects.py              # QGraphicsDropShadowEffect 카드 그림자 헬퍼 (v3.0 design-guide §5.2)
    │   └── qss.py                  # QSS 스타일시트 문자열 (컴포넌트 명세, v3.0 design-guide §6)
    ├── dialogs/
...
| `style/tokens.py` | 색상·폰트·간격·반경·그림자 토큰 (라이트/다크 팔레트, design-guide) | C-5 | ~180 |
| `style/theme.py` | ThemeManager (시스템 연동·라이트/다크 전환) | FR-5.8 | ~90 |
| `style/effects.py` | 카드 그림자 헬퍼 (QGraphicsDropShadowEffect) | C-5 | ~40 |
| `style/qss.py` | QSS 스타일시트 (컴포넌트 명세) | C-5 | ~210 |
| **View 합계** | | | **~3,000** |
...
| ResultScreen | `save_report_requested` | `()` | [v3.1] 결과 화면 하단 [리포트 저장] 클릭 → 저장 흐름(메뉴와 동일 핸들러) | FR-5.2 |
...
**[v3.1 상시 로고] (요구사항 1).** 메뉴바 **좌측 상단 끝**(`Qt.Corner.TopLeftCorner`)에 작은 프로그램 로고(`assets/logo.png`, 높이 ~20px)를 코너 위젯(`corner_logo`)으로 고정한다. 셸(QMainWindow)의 메뉴바는 어느 스택 페이지에서도 항상 보이므로, 제출·로딩·결과 모든 화면에서 좌측 상단 로고가 상시 노출된다. 로고 파일이 없으면 `#cornerLogo` 텍스트 폴백.
...
> **[v3.1] 차트 배치(세로 1열 스크롤).** v2.0의 2×2 그리드를 폐기하고 **세로 1열**로 쌓는다(요구사항 3·3-2): `막대 → 산점도 → 레이더 → 이상 신호`. 각 그래프 위에 의미를 설명하는 **섹션 타이틀(`#sectionTitle`)+힌트(`#sectionHint`)**를 붙이고(`_add_chart_section` 헬퍼), 섹션 간격은 `SPACING_SECTION`(48px)으로 넓게 둔다. **차트마다 최소 높이를 고정**(막대 360 · 산점도 420 · 레이더 440px)해 데이터 개수와 무관하게 화면 내 비율을 유지한다(요구사항 4 — 종전 그리드에서 데이터가 늘면 산점도가 작아지던 문제 해소). 스크롤은 상위 ResultScreen이 제공한다(§6.11).
...
분석 로딩 화면. 중앙 상단에 적당한 여백을 가진 작은 로고를 표시하고, 전체 화면 중앙에 스피너와 ProgressBar(§6.8)를 배치한다. 분석 시작 1초 이내 진입(NFR-1.1), 진행률 갱신, 완료/오류 시 결과/제출 화면으로 전환(전환은 Controller). 진행률 메서드는 내부 ProgressBar에 위임한다.
...
class ResultScreen(QWidget):
    ...
    save_report_requested  = pyqtSignal()        # [v3.1] 하단 [리포트 저장] (FR-5.2)
    ...
    def __init__(self) -> None:
        ...
        # QScrollArea(page) — pageTitle·banner·dashboard·merge·[새 분석|리포트 저장]을 세로로 적재
        self._save_btn.clicked.connect(self.save_report_requested)         # [v3.1]
...
두 섹션(화면, Staff Credit)은 각각 별도의 **라운드박스 패널(`COLOR_SURFACE` + `RADIUS_LG` 또는 `RADIUS_SM`)** 안에 감싸져 시각적으로 깔끔하게 분리된다.
...
> **[v2.0] 테마 연동.** BaseChartWidget은 `theme_manager.changed`(§10.2)를 구독한다. 신호 수신 시 `_apply_canvas_theme()`로 figure facecolor를 `tokens.COLOR_CANVAS`로 맞추고, 보유한 `_scores`가 있으면 `_render_static()` 후 `_draw_frame(1.0)`으로 **애니메이션 없이 최종 상태로 즉시 재렌더**한다(scores가 없으면 placeholder 재표시). 정적 요소를 그리는 하위 클래스(`_render_static`)는 축 facecolor·tick·label·spine 색을 토큰에서 읽어 라이트/다크 양쪽에서 가독성을 유지한다. 공통 축 채색은 base의 `_style_axes(ax)` 헬퍼가 담당한다.

> **[v3.0] 차트 채색(qce-design-guide §7).** `_style_axes`는 데이터가 전면에 오도록 축을 캔버스에 녹인다 — ax/figure facecolor=`COLOR_CANVAS`, 스파인=`COLOR_HAIRLINE`(lw 0.8), 눈금=`COLOR_TEXT_MUTED`(labelsize 11). 막대 1위는 `COLOR_BAR_TOP`(=PRIMARY)·그 외 `COLOR_BAR_DEFAULT`, 평균선/십자선 `COLOR_AVG_LINE`, 그리드 `COLOR_GRID`, 이상 점 `COLOR_ANOMALY`, 사분면 배경 `QUADRANT_COLORS`. matplotlib 폰트는 `FONT_FAMILY_CHART`(Malgun Gothic)로 지정하고 `axes.unicode_minus=False`로 음수 부호 깨짐을 막는다.
...
팀원별 종합 기여 지표 막대. **Y축은 데이터가 분포한 범위를 중심으로 80% 확대(왜곡형 그래프 방식)**하여 미세한 차이도 식별하기 쉽게 한다. 그리드 0.2, 1위 강조색, 평균선, 수치 레이블, 6항목 툴팁, 하단→최종 높이 상승 애니메이션.
...
3. **종합 그래프 + 인원별 개인 그래프 병렬.** 한 figure 안에 **종합 레이더(전체 팀원 + 팀 평균 오버레이)**와 **팀원 1인당 개인 레이더**(해당 팀원 폴리곤 + 팀 평균 옅은 점선)를 한 행으로 나란히 배치한다. 레이아웃은 `[0.5, 3.7, 1.6, 3.7, 0.5]` GridSpec(1열=종합 여백, 2열=종합, 3열=범례/여백, 4열=개인, 5열=개인 여백)으로 넓게 구성한다. 또한 `set_in_layout(False)`로 텍스트 영역을 레이아웃 계산에서 배제해, 순수 도형이 3.7 비율을 온전히 차지하도록 보정한다. 개인 레이더 행(row) 개수에 따라 동적으로 높이가 증가한다.
...
**[v2.1] 가용 데이터 수 기반 동적 산점도.** 입력 데이터 개수에 따라 표시 방식이 변한다: (1개) 회색 텍스트 "자료가 한 종류인 경우 산점도가 계산되지 않습니다." 표시, (2개) Y축 첫 번째/X축 두 번째 소스, (3개) Y축 첫 번째/X축 두 번째 소스 + 점 색상 세 번째 소스 점수 비례(노란색부터 보라색까지의 그라데이션) 및 우측 컬러바 범례 표시. 
산점도와 컬러바의 전체 영역 대비 **가로세로 비율을 항상 3:2**로 고정하고, **컬러바의 비율은 1:10**으로 강제(`aspect=10`)하여 비율이 일그러지지 않게 한다. 점이 잘리는 현상을 막기 위해 `clip_on=False`를 적용한다. 십자선(해당 축 평균, axvline/axhline 사용), 동적 툴팁, 라벨 겹침 4방향 해소, fade-in 애니메이션, 하위 이상치 점 경고 표시. **점 클릭 시 `member_selected(author)` 발행**(결정 B; 누가 듣는지 모름).
...
## 10. 스타일·리소스 토큰·테마 (FR-5.8, qce-design-guide v1.0)

색상·폰트·간격·반경·그림자를 `style/tokens.py`에 단일 출처로 두고, QSS 문자열은 `style/qss.py`가 토큰을 보간해 생성한다. matplotlib 차트 색상도 동일 토큰을 참조해 UI와 일관성을 유지한다. **[v3.0]** 디자인 양식은 `qce-design-guide.md`(Product-First Minimalism)를 단일 출처로 따른다 — 흑백 계조 위에 **단일 액센트 블루**, **8px 배수 간격 그리드**, **3종 반경 문법**(유틸 6px·카드 12px·주 버튼 pill), **단일 그림자 토큰**(카드·메인 패널 전용). 기능·데이터·신호/슬롯 계약은 v2.0과 동일하며 **표현 속성(색·여백·반경·그림자)만** 교체되었다.

### 10.1 라이트/다크 팔레트 (v3.0, qce-design-guide §2·§9)

`tokens.py`는 색상명을 **모듈 전역 상수**로 노출하되, 그 값은 활성 팔레트(라이트/다크)에 따라 바뀐다. 두 팔레트를 dict(`_LIGHT`·`_DARK`)로 정의하고, `apply_palette(dark: bool)`가 활성 팔레트의 값으로 모듈 전역을 **재바인딩**한다. 차트·QSS는 모두 `tokens.COLOR_X`처럼 **속성 접근**으로 읽으므로(import-time 캡처가 아님), 재바인딩 직후 다음 렌더·QSS 생성에 새 색이 반영된다. 간격·반경·폰트(`SPACING_*`·`RADIUS_*`·`FONT_*`)는 테마 무관 상수다.

> **하위 호환.** 신규 토큰명(`COLOR_CANVAS`·`COLOR_TEXT_MUTED` 등)을 도입하되, 기존 차트·테스트가 쓰던 `COLOR_BG`(=`COLOR_CANVAS`)·`COLOR_MUTED`(=`COLOR_TEXT_MUTED`)를 **별칭으로 병행 노출**해 변경 없이 동작하게 한다.

```python
# style/tokens.py (발췌) — Product-First Minimalism 팔레트
_LIGHT = {
    "COLOR_CANVAS": "#ffffff", "COLOR_SURFACE": "#f5f5f7", "COLOR_SURFACE_2": "#fafafc",
    "COLOR_TEXT": "#1d1d1f", "COLOR_TEXT_MUTED": "#6e6e73", "COLOR_TEXT_DISABLED": "#aeaeb2",
    "COLOR_PRIMARY": "#0066cc", "COLOR_PRIMARY_FOCUS": "#0071e3",   # 유일 액센트
    "COLOR_HAIRLINE": "#e0e0e0", "COLOR_DIVIDER_SOFT": "rgba(0,0,0,0.06)",
    "COLOR_WARNING_BG": "#fff8e1", "COLOR_WARNING_FG": "#5f4300", "COLOR_ANOMALY": "#cc0000",
    "COLOR_BAR_TOP": "#0066cc", "COLOR_BAR_DEFAULT": "#adb5bd",
    "COLOR_AVG_LINE": "#5f6368", "COLOR_GRID": "#e0e0e0",
    "QUADRANT_COLORS": {"올라운더": "#e8f4fd", "개발 집중": "#e8f0fe",
                        "문서 집중": "#fef0e3", "저참여": "#f1f3f4"},
    "SHADOW_CARD": "0 2px 12px rgba(0,0,0,0.10)", "SHADOW_CARD_ALPHA": 26,
    "COLOR_BG": "#ffffff", "COLOR_MUTED": "#6e6e73",   # 하위 호환 별칭
}
_DARK = {
    "COLOR_CANVAS": "#1c1c1e", "COLOR_SURFACE": "#2c2c2e", "COLOR_SURFACE_2": "#3a3a3c",
    "COLOR_TEXT": "#f5f5f7", "COLOR_TEXT_MUTED": "#aeaeb2", "COLOR_TEXT_DISABLED": "#636366",
    "COLOR_PRIMARY": "#2997ff", "COLOR_PRIMARY_FOCUS": "#409cff",   # 다크 가시성 확보
    "COLOR_HAIRLINE": "#3a3a3c", "COLOR_DIVIDER_SOFT": "rgba(255,255,255,0.08)",
    "COLOR_WARNING_BG": "#3a2f00", "COLOR_WARNING_FG": "#ffd60a", "COLOR_ANOMALY": "#ff6b6b",
    "COLOR_BAR_TOP": "#2997ff", "COLOR_BAR_DEFAULT": "#636366",
    "COLOR_AVG_LINE": "#9aa0a6", "COLOR_GRID": "#3c3c3e",
    "QUADRANT_COLORS": {"올라운더": "#1a2e3d", "개발 집중": "#1a2340",
                        "문서 집중": "#2d2010", "저참여": "#252526"},
    "SHADOW_CARD": "0 2px 12px rgba(0,0,0,0.40)", "SHADOW_CARD_ALPHA": 102,
    "COLOR_BG": "#1c1c1e", "COLOR_MUTED": "#aeaeb2",   # 하위 호환 별칭
}

# 테마 무관 상수 (qce-design-guide §3·§4·§5)
FONT_FAMILY = "Malgun Gothic, Segoe UI, sans-serif"   # UI(QSS)
FONT_FAMILY_CHART = "Malgun Gothic"                    # matplotlib rcParams(C-5)
FONT_HERO, FONT_TITLE, FONT_SUBTITLE, FONT_BODY, FONT_CAPTION = 28, 20, 17, 15, 13
SPACING_XS, SPACING_SM, SPACING_MD, SPACING_LG, SPACING_XL, SPACING_SECTION = 8, 12, 16, 24, 32, 48
RADIUS_SM, RADIUS_LG, RADIUS_PILL = 6, 12, 9999
TOUCH_TARGET_MIN, CARD_PADDING, DROPZONE_MIN_H, PROGRESS_H = 44, 24, 160, 6
GRID_STEP = 0.2
...
> **컴포넌트 QSS·objectName 계약(§6 design-guide).** `qss.py`가 토큰을 보간해 컴포넌트별 규칙을 생성한다. 위젯은 objectName으로 규칙을 받는다: 주 버튼 `#primary`(pill), 세컨더리/유틸 `#secondary`(6px), 프리셋 칩 `#preset`(pill), 카드 `#card`(12px+surface), 드롭존 `#dropzone`(점선, `[drag_active="true"]` 강조), 가중치 슬라이더 `#weight`(핸들은 **깔끔한 사각형** — 14×18px·`border-radius:3px`, 요구사항 2), 진행률 `#analysis`(6px), 경고 배너 `#warningBanner`, 페이지/섹션 타이틀 `#pageTitle`·`#sectionTitle`·`#sectionHint`, 셸 좌측 상단 로고 `#cornerLogo`, 보조 라벨 `#placeholder`, 가중치 경고 `#weightWarning`, 신호 카드 `#signalCard`(COLOR_ANOMALY 좌측 보더)·섹션 `#signalSection`·해제 버튼 `#dismissSignal`. 그림자는 QSS가 표현하지 못하므로 카드·메인 패널에 한해 `style/effects.apply_card_shadow(widget)`가 `QGraphicsDropShadowEffect`(SHADOW_CARD)를 부착한다(버튼·텍스트·헤더 금지).
...
| **v3.0** | **2026-06-02** | **`qce-design-guide.md` v1.0(Product-First Minimalism) 적용 — 표현 속성 전면 개편(기능·데이터·신호/슬롯 계약 무변경). (1) §10을 design-guide 토큰 체계로 전면 재작성: 흑백 계조 + 단일 액센트 블루(`#0066cc`/다크 `#2997ff`), 라이트/다크 팔레트에 `COLOR_CANVAS`·`COLOR_SURFACE(_2)`·`COLOR_TEXT(_MUTED/_DISABLED)`·`COLOR_PRIMARY(_FOCUS)`·`COLOR_HAIRLINE`·`COLOR_DIVIDER_SOFT`·`COLOR_BAR_TOP`·`SHADOW_CARD` 추가(기존 `COLOR_BG`·`COLOR_MUTED`는 별칭 병행). 8px 간격(`SPACING_*`)·3종 반경(`RADIUS_SM/LG/PILL`)·타이포 계층(`FONT_HERO`…`FONT_FINE_PRINT`)·`FONT_FAMILY_CHART` 상수 도입. (2) §3에 `style/effects.py`(카드 그림자 헬퍼) 추가, LOC 표·합계(~3,000) 갱신, `qss.py`를 컴포넌트 명세(`#primary` pill·`#secondary`·`#preset`·`#card`·`#dropzone`·`#weight`·`#analysis`·`#signalCard` 등 objectName 계약)로 확장. (3) §7.1에 design-guide §7 차트 채색 헬퍼(`_style_axes`=캔버스 융화·헤어라인 스파인·뮤트 눈금) 추가. (4) `theme.py` QPalette·`test_settings_theme.py` 팔레트 검증값을 신규 팔레트로 갱신. 위젯 합성·Signal/슬롯·테스트 표면(접근자)은 모두 그대로 유지.** | QCE 개발팀 (View 담당 이대한) |
| **v3.1** | **2026-06-02** | **사용자 피드백(결과 화면 가독성·상시 로고·슬라이더·차트 비율) 반영 — 표현/배치 변경(기능·데이터 무변경). (1) **상시 로고(요구사항 1)**: §6.1 MainWindow 메뉴바 좌측 상단 코너에 작은 로고(`corner_logo`/`#cornerLogo`) 고정 — 전 화면 상시 노출. 결과 화면의 중앙 로고는 제거(페이지 타이틀로 대체). (2) **슬라이더 핸들(요구사항 2)**: `#weight` 핸들을 원형→깔끔한 사각형(14×18·radius 3)으로 변경. (3) **결과 화면 재구성(요구사항 3·3-2)**: §6.11 ResultScreen을 `QScrollArea` 세로 스크롤 페이지로 — 넉넉한 좌우 패딩(`SPACING_SECTION`)·섹션 간 넓은 간격, 순서 `페이지 타이틀("프로젝트 분석 결과") → 배너 → 막대 → 산점도 → 레이더 → 이상 신호 → 인원 합치기 → [새 분석|리포트 저장]`. §6.6 DashboardView를 2×2 그리드→세로 1열로 바꾸고 그래프마다 섹션 타이틀(`#sectionTitle`)+힌트(`#sectionHint`) 부여(`_add_chart_section`). 하단 [리포트 저장] 버튼 신설 → `save_report_requested`(§5.1) 추가, Controller가 메뉴와 동일 핸들러에 연결. (4) **차트 비율 고정(요구사항 4)**: 차트별 최소 높이 고정(막대 360·산점도 420·레이더 440px)으로 데이터 개수와 무관하게 비율 유지(산점도 축소 문제 해소). QSS에 `#pageTitle`·`#sectionTitle`·`#sectionHint`·`#cornerLogo` 규칙 추가.** | QCE 개발팀 (View 담당 이대한) |
| **v3.2** | **2026-06-02** | **UI 폴리싱 2차 반영: (1) 막대 그래프 Y축 확대(`set_ylim`), 산점도 비율 3:2 고정, 레이더 차트 텍스트 무시 3.7 GridSpec 보정 등 §7.2/7.3/7.4 차트 명세 갱신. (2) §6.9/§6.11 화면 크기에 비례하는 동적 패딩(15%, 10%) 및 대형 로고 추가. (3) §6.13 설정 모달 섹션 패널 분리 사양 갱신.** | 이대한, 김휘중 공동 작업 |
