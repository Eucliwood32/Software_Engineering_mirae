...
| 5 | **반경 문법** | 주 버튼/유틸 버튼=6px(라운드 박스), 카드=12px. 이 값 외 중간값 금지. (Pill은 제한적으로 사용) |
...
### SubmitScreen (제출)
- 배경 `COLOR_CANVAS`
- 상단 로고: 대형 `QCE_Logo.png` 픽스맵(높이 120px) + 서브타이틀("QCE - 부탁해 꼬마선장") 배치
- 여백: 동적 화면 패딩(좌우 폭의 15%, 상하 높이의 10%) 적용으로 전체화면 대응
- 드롭존: `COLOR_SURFACE` 배경 + 점선 테두리 (캔버스 위에 살짝 올라온 느낌)
- AnalysisPanel: `COLOR_SURFACE` 카드로 감싸 드롭존과 구분
- [분석 시작]: 가운데 정렬, 둥근 사각형(RADIUS_SM), `COLOR_PRIMARY` — 화면의 유일한 강조 버튼
...
### ResultScreen (결과)
- 배경 `COLOR_CANVAS`
- 레이아웃: 진입 시 스크롤 최상단 강제 초기화, 화면에 비례하는 동적 패딩(좌우 15%, 상하 10%) 적용
- 대시보드: 차트가 주어진 폭 사용, 타이틀 하단 여백 축소(0.5배)로 밀도 상향
- WarningBanner·신호 패널: `COLOR_SURFACE` 카드 안
- 설정(SettingsDialog) 모달: 화면, Staff Credit을 각각 라운드박스 패널로 분리
...
| v1.1 | 2026-06-02 | UI 폴리싱 2차 반영에 따른 동적 패딩 비율, 컴포넌트 간격/반경 규칙(라운드 박스), 대형 로고 업데이트 등 최신 레이아웃 동기화. | 이대한, 김휘중 공동 작업 |
